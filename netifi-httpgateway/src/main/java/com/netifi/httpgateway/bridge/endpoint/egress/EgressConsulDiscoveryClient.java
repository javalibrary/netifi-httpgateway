...
public class EgressConsulDiscoveryClient {
          ...
          .map(
              serviceName ->
                  new AbstractMap.SimpleEntry<>(
                      serviceName, getHealthyServiceEndpoints(serviceName)))
  ...
  private Set<EgressDiscoveryEndpoint> getHealthyServiceEndpoints(String serviceName) {
    ConsulResponse<List<ServiceHealth>> serviceHealthResponse =
    ...
    if (serviceHealthResponse != null) {
      ...
      return serviceHealths.stream()
          .map(
              serviceHealth ->
                  EgressDiscoveryEndpoint.newBuilder()
                      .withHost(serviceHealth.getService().getAddress())
                      .withPort(serviceHealth.getService().getPort())
                      .build())
