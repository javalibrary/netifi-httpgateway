...
public abstract class HttpObjectDecoder extends ByteToMessageDecoder {
  ...
  /**
   * The internal state of {@link io.netty.handler.codec.http.HttpObjectDecoder}. <em>Internal use
   * only</em>.
   */
  private enum State {
    SKIP_CONTROL_CHARS,
  ...
  protected HttpObjectDecoder(
    ...
    if (maxInitialLineLength <= 0) {
      throw new IllegalArgumentException(
          "maxInitialLineLength must be a positive integer: " + maxInitialLineLength);
    }
    if (maxHeaderSize <= 0) {
      throw new IllegalArgumentException(
  ...
  @Override
  public void decode(ByteBuf buffer, UnicastProcessor<Object> out) {
    if (resetRequested) {
      resetNow();
    }
    ...
    switch (currentState) {
      ...
      case READ_INITIAL:
        try {
          ...
          out.onNext(invalidMessage(buffer, e));
          return;
        }
      case READ_HEADER:
        try {
          ...
          switch (nextState) {
            case SKIP_CONTROL_CHARS:
              ...
              out.onNext(message);
              out.onNext(LastHttpContent.EMPTY_LAST_CONTENT);
              resetNow();
              return;
      ...
      case READ_VARIABLE_LENGTH_CONTENT:
        {
          ...
          if (toRead > 0) {
            ...
            out.onNext(new DefaultHttpContent(content));
          }
          return;
        }
      case READ_FIXED_LENGTH_CONTENT:
        {
          ...
          if (chunkSize == 0) {
            // Read all content.
            out.onNext(new DefaultLastHttpContent(content, validateHeaders));
            resetNow();
          } else {
            out.onNext(new DefaultHttpContent(content));
          }
      ...
      case READ_CHUNK_SIZE:
        try {
          ...
          out.onNext(invalidChunk(buffer, e));
          return;
        }
      ...
      case UPGRADED:
        {
          ...
          if (readableBytes > 0) {
            ...
            out.onNext(buffer.readBytes(readableBytes));
          }
          break;
        }
    }
  ...
  @Override
  public void decodeLast(ByteBuf in, UnicastProcessor<Object> out) {
    super.decodeLast(in, out);
   ...
   * HTTP/2 or Websocket. Returns false if the upgrade happened in a different layer, e.g. upgrade
   * from HTTP/1.1 to HTTP/1.1 over TLS.
   */
